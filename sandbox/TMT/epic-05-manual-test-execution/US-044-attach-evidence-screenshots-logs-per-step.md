# User Story: Attach evidence (screenshots, logs) per step
...
**As a** tester
**I want** to attach evidence (screenshots, logs) per step
**So that** release and coverage decisions can rely on current execution evidence

---

## Context
Attach evidence (screenshots, logs) per step exists so release and coverage decisions can rely on current execution evidence. It belongs to the Manual Test Execution backlog and should keep project scope, traceability, and operability clear while TMT grows.
...
### Scenario 1: Complete Attach evidence (screenshots, logs) per step
**Given** a tester is working inside the correct TMT project and is ready to attach evidence (screenshots, logs) per step
**When** they attach evidence (screenshots, logs) per step
...
**Given** required input, mappings, or permissions for attach evidence (screenshots, logs) per step are missing
**When** the tester tries to attach evidence (screenshots, logs) per step
