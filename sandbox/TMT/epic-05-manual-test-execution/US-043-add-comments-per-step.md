...
## Context
Add comments per step exists so release and coverage decisions can rely on current execution evidence. It belongs to the Manual Test Execution backlog and should keep project scope, traceability, and operability clear while TMT grows.
...
### Scenario 1: Complete Add comments per step
**Given** a tester is working inside the correct TMT project and is ready to add comments per step
**When** they add comments per step
...
**Given** required input, mappings, or permissions for add comments per step are missing
**When** the tester tries to add comments per step
