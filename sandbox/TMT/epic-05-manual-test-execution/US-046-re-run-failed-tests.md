# User Story: Re-run failed tests
...
**As a** tester
**I want** to re-run failed tests
**So that** release and coverage decisions can rely on current execution evidence

---

## Context
Re-run failed tests exists so release and coverage decisions can rely on current execution evidence. It belongs to the Manual Test Execution backlog and should keep project scope, traceability, and operability clear while TMT grows.
...
### Scenario 1: Complete Re-run failed tests
**Given** a tester is working inside the correct TMT project and is ready to re-run failed tests
**When** they re-run failed tests
...
**Given** required input, mappings, or permissions for re-run failed tests are missing
**When** the tester tries to re-run failed tests
