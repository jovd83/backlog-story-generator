# User Story: Store automation metadata (framework, repo, path)
...
**As a** tester
**I want** to store automation metadata (framework, repo, path)
**So that** release and coverage decisions can rely on current execution evidence

---

## Context
Store automation metadata (framework, repo, path) exists so release and coverage decisions can rely on current execution evidence. It belongs to the Automated Test Execution backlog and should keep project scope, traceability, and operability clear while TMT grows.
...
### Scenario 1: Complete Store automation metadata (framework, repo, path)
**Given** a tester is working inside the correct TMT project and is ready to store automation metadata (framework, repo, path)
**When** they store automation metadata (framework, repo, path)
...
**Given** required input, mappings, or permissions for store automation metadata (framework, repo, path) are missing
**When** the tester tries to store automation metadata (framework, repo, path)
